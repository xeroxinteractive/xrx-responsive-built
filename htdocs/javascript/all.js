...
$('.carousel-indicators li').click(function() {
	...
	if (typeof xrx_hbx_proxy !== 'undefined') {
		...
		xrx_hbx_proxy.xrxLid("bs-carousel-"+$clickedCarouselID+"-"+$ordinal_target_slide_number);
	}
});
	  ...
	  success: function (data) {
	  	...
	  	for (i = 0; i < data.length; i++) {
	  	    ...
	  	    obj.image_link = obj.image_link || '';
	  	   	...
	  	   	$timeago = moment(obj.updated).twitter();
	  	   	...
	  	   	if ($medium === 'twitter') {
	  	   		...
	  	   		$('.social-side .twitter').append('<div data-index="' + $index + '"><p class="account"><a href="' + obj.account_link + '" target="_blank">' + obj.account_name + ' ' + obj.account + '</a> ' + $timeago + '</p><p>' + $title + '</p><div class="twitter-actions">' + $reply + ' ' + $retweet + ' ' + $favorite + '</div></div>');
	  	   		...
	  	   		$('.social-side .linkedin').append('<div data-index="' + $index + '">' + $image + '<div class="title">' + $title + '</div><div class="author">' + obj.account + '</div><p>' + $excerpt + '</p></div>');
