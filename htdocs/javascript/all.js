...
function xrx_sitecatalyst_action($action_label) {
	...
	} else if (typeof xrx_sc_synthetic_lid_event == 'function') {
...
if ($('.homepage-promos').length) {
		var $label,
		$labelTitle;
		// if the adobe target code works we wait for that image to load

		if ($('.homepage-promos .mboxDefault').is(':hidden')) {
				// active container worked
				jQuery.scrollDepth({
				  userTiming: false,
				  pixelDepth: false,
				  percentage: false,
				  elements: ['.homepage-promos #active-container-tmpl .hero-promo', '.homepage-promos #active-container-tmpl .promos .small-promo-a',  '.homepage-promos #active-container-tmpl .promos .resource-desk', '.social-container .media-feature', '.social-container .blog-side' , '.social-container .social-side .twitter .tweet', '.social-container .social-side .linkedin .linkedin-text'],
		...
		// if the target code fails we use the default image to detect load
			    ...
			    jQuery.scrollDepth({
			      ...
			      pixelDepth: false,
			      percentage: false,
			      elements: ['.homepage-promos .mboxDefault .hero-promo', '.homepage-promos .mboxDefault .promos .small-promo-a', '.homepage-promos .mboxDefault .promos .resource-desk', '.social-container .media-feature', '.social-container .blog-side' , '.social-container .social-side .twitter .tweet', '.social-container .social-side .linkedin .linkedin-text'],
			      eventHandler: function(data) {
...
function autoPlayYouTubeModal(){
  ...
  trigger.click(function(e) {
    ...
    videoSRCauto = videoSRC+"?autoplay=1&modestbranding=1&theme=dark&showinfo=0&autohide=1&playsinline=0&rel=0" ;
    $(theModal+' iframe').attr('src', videoSRCauto);
  if (typeof s_YTi !== 'undefined') {
