 ...
 //console.log($('html').attr('data-test'));
...
/**
* MOVE ALL THE BLOG PARAMS TO ATTRIBUTES ON THE DOM FOR THAT COMPONENTS.  THIS IS SOURCE AND COUNT.  THIS CAN CHANGE PER LANGUAGE. GET RID OF THE PRETTY VAR.
**/
...
function blogBuilder(obj) {
	...
	if ($post.excerpt.length > 100) {
		...
		/** I DO NOT THINK WE CAN HAVE ANY ENGLISH WORDS LIKE COMMENTS IN HERE FOR DIFFERENT LANGUAGES.  CAN THIS BE COMBINED INTO ONE FUNCTION?  IS THIS THE SAME DOM ELEMENT BEING MADE IN THE FUNCTION ABOVE? **/
...
// Controls the video poster play
  if (!isMobile()){
  $('#videoPoster').click(function() {    /*IS THERE A WAY WE CAN WRITE THIS IN NATIVE JS SINCE THERE IS ISSUES WHEN WE WRAP THIS FILE IN JQUERY.  I GET SCARED.*/
    if (!isIE8()) {
      $('#player').show();
     }
    player.playVideo();
