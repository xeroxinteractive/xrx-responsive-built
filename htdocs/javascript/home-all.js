...
(function($) {
	if ($(".blog-side")[0]) {
		...
		function blogBuilder(obj) {
				...
				if (obj.image_link) {
					$post.image = '<img src="' + obj.image_link + '" alt="' + obj.title + '" />';
				} else {
					var $fallbackImage = $('.blog-side').attr('data-fallback-image');
					$post.image = '<img src="' + $fallbackImage + '"  alt="' + obj.title + '"  />';
				}
			...
			$(function() {
			  ...
			  $('.blog-select').on('change', function(){
			    $selectedBlog = $(this).find("option:selected").val(),
			    $selectedBlogTitle = $(this).find("option:selected").text();
			    if ($selectedBlog === 'all') {
			    	allBlogLoop();
			    }
			    else {
...
//resource desk
(function($) {
    if ($(".rd-select-wrapper")[0]) {

        $('html').addCLass('homepage-height-fix');
        ...
        $('.close-elicit').click(function(e) {
            ...
            $('.AC-active select[name=rd-selector]').val(0);
            $('.AC-active .rd-selector').selectpicker('refresh');
