...
/****************** List of images **********************/
/*
Hero Desktop: hero-desktop.jpg
Hero Mobile: hero-mobile.jpg
Promo A: small-promo-a.jpg
Promo B: small-promo-b.jpg
*/

if ($(".promo-json")[0]) {
var $acJSON = $('.promo-json').html(),
$promoPersona = $('.promo-json').attr('data-persona'),
$acParse = jQuery.parseJSON($acJSON),
$personaFound = false;

$.each( $acParse, function( key, value ) {
 	//console.log( key + ": " + value );
 	if (key === $promoPersona) {

 	var $type = key,
 	obj = value;
 	obj.heroLarge = obj.heroLarge || '';
 	obj.heroLarge.ctaCopy = obj.heroLarge.ctaCopy || '';
 	var $promos = {};
 	// hero large
 	...
 	$promos.smallPromoAColor = obj.smallPromoA.color;
	...
	// locale 
	$promos.locale = $('.promo-json').attr('data-locale');

 	//$personaFound = true;
 	// Appends the content 
 	var template = $.templates("#active-container-tmpl-js");

 	var htmlOutput = template.render($promos);
...
(function($) {
    if ($(".rd-selection")[0]) {
        ...
        $('.rd-selection ul').on('click', 'a', function (e) {
            ...
            $('.rd-selected-clone span').text($selectedRDTitle);
        ...
        function getResults() {
                $.ajax({
                        type: "GET",
                        dataType: "jsonp",
                        url: 'http://xerox.elicitapp.com/s.ashx?q=' + $curKeyword + '%20&r=17&sn=' + $sn + '&su=' + $su + '&l=' + $l + '&p=0&f=' + $f + '&_=1421247885992'
