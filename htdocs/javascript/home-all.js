...
(function($) {
    if ($(".rd-selection")[0]) {
        ...
        function getResults() {
            console.log('http://xerox.elicitapp.com/s.ashx?q=' + $curKeyword + '%20&r=17&sn=' + $sn + '&su=' + $su + '&l=' + $l + '&p=0&f=' + $f + '&_=1421247885992');
                $.ajax({
                        type: "GET",
                        dataType: "jsonp",
                        ...
                        if (msg.cs) {
                            for (var k = 0, len = msg.cs.length; k < len; k++) {
                                ...
                                for (var i = 0, l = msg.cs[k].rs.length; i < l; i++) {
                                    ...
                                    var $url = 'http://www.xerox.com/search?q=' + $curKeyword + '&js_avail=1&locale=' + $l ;
